...
public partial class page_admin3 : System.Web.UI.Page
{
    ...
    {
        if (Convert.ToString(Session["identity"]) == null || Convert.ToString(Session["identity"]) == "")
        {
    ...
    protected void submitForm(string userId, string userName, string userPasswd, string userPosition, string userPhone, string userEmail, string BelongTo)
    {
        ...
        OleDbConnection conn = Getconnection();
        //第二步：这里写入你需要执行的SQL语句 
        string sql = "INSERT INTO worker_account(ID_worker,_Name,_Passwd,_Position,_PhoneNum,_Email,ID_College) VALUES('" + userId + "','" + userName + "','" + userPasswd + "','" + userPosition + "','" + userPhone + "','" + userEmail + "','" + BelongTo + "')";
        //第三步：执行数据库命令传送 且 返回一个处理结果对象
        OleDbCommand cmd = new OleDbCommand(sql, conn);
        //第四步：打开conn对象建立数据库连接
        conn.Open();
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                Response.Write("<script>alert('该科长已经存在，请检查输入的科长ID是否重复');</script>");
            }
            else
            {
                Response.Write("<script>alert('插入成功！');</script>");
                //传入参数：userId 用户ID , userName 用户姓名 ,userPasswd 密码,userPosition职位 userPhone 用户手机号 , userEmail 用户邮箱 , _BelongTo所属
                submitForm(TextBox1.Text, TextBox2.Text, TextBox3.Text, TextBox4.Text, TextBox5.Text, TextBox6.Text, TextBox6.Text);
            }
    ...
    {
        Response.Write("<script>alert('所需要删除的科长不存在！');</script>");
