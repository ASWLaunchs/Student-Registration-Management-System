<%@ Page Language="C#" AutoEventWireup="true" CodeFile="admin1.aspx.cs" Inherits="page_admin1" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title></title>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <!-- https://fonts.google.com/specimen/Roboto -->
    <link rel="stylesheet" href="../css/fontawesome.min.css" />
    <!-- https://fontawesome.com/ -->
    <link rel="stylesheet" href="../css/bootstrap.min.css" />
    <!-- https://getbootstrap.com/ -->
    <link rel="stylesheet" href="../css/templatemo-style.css" />
    <link rel="stylesheet" href="../css/search.css" />
    <!--
	Product Admin CSS Template
	https://templatemo.com/tm-524-product-admin
	-->
    <style>
        body {
            height:100vh;
            background-color:#2c3944;
        }

        .Form {
            margin-top: 40px;
            color: #fff;
        }

            .Form table th, .Form table td {
                color: #fff;
                padding: 8px 20px;
            }

        [id^="searchView" ] {
            height: 100vh;
        }

        .nav-link {
            padding-left: 0px !important;
        }

        #close3 {
            position: fixed;
            top: 20px;
            right: 20px;
            transition: all 0.3s ease;
            cursor: pointer;
        }

            #close3:hover {
                transform: scale(1.2);
            }

                #close3:hover path {
                    fill: coral;
                }

        #close4 {
            position: fixed;
            top: 20px;
            right: 20px;
            transition: all 0.3s ease;
            cursor: pointer;
        }

            #close4:hover {
                transform: scale(1.2);
            }

                #close4:hover path {
                    fill: coral;
                }

        .modify {
        }

        .container {
            width: 96vw;
        }

        .tm-footer{
            position:absolute;
            bottom:0px;
        }

        .tm-block{
            max-height:560px !important;
        }

    </style>
</head>

<body id="reportsPage">
    <form runat="server" class="Form">
        <div class="" id="home">
            <nav class="navbar navbar-expand-xl">
                <div class="container h-100">
                    <a class="navbar-brand" href="javascript:;">
                        <h1 class="tm-site-title mb-0">
                            <asp:Label ID="Label1" runat="server" Text="Label"><p class="text-white mt-5 mb-5">欢迎回来, <b>科长</b></p></asp:Label>
                        </h1>
                    </a>
                    <button class="navbar-toggler ml-auto mr-0" type="button" data-toggle="collapse" data-target="#navbarSupportedContent"
                        aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
                        <i class="fas fa-bars tm-nav-icon"></i>
                    </button>

                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul class="navbar-nav mx-auto h-100">
                            <li class="nav-item">
                                <a class="nav-link" href="javascript:;">
                                    <button id="yourselfInfo" type="button" class="btn btn-primary btn-lg">
                                        <svg t="1607845567223" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2921" width="24" height="24">
                                            <path d="M511.554783 534.26087c123.458783 0 229.331478 18.075826 306.44313 53.648695C906.150957 628.557913 957.217391 692.713739 957.217391 774.678261c0 83.389217-52.402087 140.02087-144.027826 170.740869-53.915826 18.120348-122.434783 28.226783-209.341217 32.144696l79.738435-75.375304a17.808696 17.808696 0 0 0 3.828869-20.658087c38.422261-4.763826 70.745043-11.575652 97.413565-20.48 58.63513-19.678609 83.255652-46.302609 83.255653-86.372174 0-43.186087-28.226783-78.625391-87.440696-105.961739-55.02887-25.377391-132.140522-40.737391-223.766261-44.521739l31.031652 31.165217a22.26087 22.26087 0 0 1 0 31.47687l-43.364174 43.27513 55.785739 180.090435a17.808696 17.808696 0 0 1-4.763826 18.253913L523.575652 996.396522a17.808696 17.808696 0 0 1-24.486956 0l-71.991653-68.029218a17.808696 17.808696 0 0 1-4.763826-18.209391l55.696696-180.134956-43.27513-43.230609a22.26087 22.26087 0 0 1 0-31.47687l31.076174-31.120695c-91.269565 3.82887-167.980522 19.144348-222.742261 44.521739-59.035826 27.247304-87.173565 62.686609-87.173566 105.917217 0 40.114087 24.486957 66.738087 82.854957 86.372174 26.490435 8.904348 58.501565 15.62713 96.612174 20.435478a17.630609 17.630609 0 0 0 3.695304 20.702609l79.693913 75.375304c-86.505739-3.962435-154.757565-14.06887-208.450782-32.144695C118.962087 914.69913 66.782609 858.022957 66.782609 774.678261c0-81.92 50.93287-146.120348 138.907826-186.768696C282.579478 552.336696 388.274087 534.26087 511.554783 534.26087zM512 0a244.869565 244.869565 0 1 1 0 489.73913 244.869565 244.869565 0 0 1 0-489.73913z m0 89.043478a155.826087 155.826087 0 1 0 0 311.652174 155.826087 155.826087 0 0 0 0-311.652174z" p-id="2922" fill="#ffffff"></path></svg>
                                        <span style="font-size: 14px; letter-spacing: 2px; text-indent: 4px">您的信息</span>
                                    </button>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="#">
                                    <button id="dataCheck" type="button" class="btn btn-primary btn-lg">
                                        <svg t="1607844228038" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3202" width="24" height="24">
                                            <path d="M161.933635 982.016249A88.059387 88.059387 0 0 1 73.142857 894.932048V87.087126A88.059387 88.059387 0 0 1 161.933635 0.002926h396.267243v71.042374H161.933635a16.090586 16.090586 0 0 0-15.846789 16.041826v807.844922a16.090586 16.090586 0 0 0 15.846789 16.041827v71.042374zM774.984951 112.978341H238.92465a48.759351 48.759351 0 0 0-50.856003 45.492474v819.986001A48.759351 48.759351 0 0 0 238.92465 1023.94929h661.030517A48.759351 48.759351 0 0 0 950.81117 978.456816V286.51287zM442.39742 892.299043a17.9922 17.9922 0 0 1-18.235997 17.748404H358.872652a17.9922 17.9922 0 0 1-18.235997-17.748404v-101.126893a17.9922 17.9922 0 0 1 18.235997-17.748404H424.210182a17.9922 17.9922 0 0 1 18.235997 17.748404V892.299043z m178.069149 3.120599a15.066639 15.066639 0 0 1-15.212918 14.627805H533.723684a15.115399 15.115399 0 0 1-15.212918-14.627805V423.770443a15.115399 15.115399 0 0 1 15.212918-14.627806h71.529967a15.115399 15.115399 0 0 1 15.212918 14.627806v471.600439z m177.92287 3.900748a11.068373 11.068373 0 0 1-11.21465 10.727057h-79.380223a11.068373 11.068373 0 0 1-11.214651-10.922095v-251.451971a11.068373 11.068373 0 0 1 11.214651-10.922095h79.331463a11.117132 11.117132 0 0 1 11.214651 10.922095v251.647009z" fill="#ffffff" p-id="3203"></path></svg></svg>
                                        <span style="font-size: 14px; letter-spacing: 2px; text-indent: 4px">辅导员信息统计</span>
                                    </button>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="#">
                                    <button id="dataStatistics" type="button" class="btn btn-primary btn-lg">
                                        <svg t="1607844228038" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3202" width="24" height="24">
                                            <path d="M161.933635 982.016249A88.059387 88.059387 0 0 1 73.142857 894.932048V87.087126A88.059387 88.059387 0 0 1 161.933635 0.002926h396.267243v71.042374H161.933635a16.090586 16.090586 0 0 0-15.846789 16.041826v807.844922a16.090586 16.090586 0 0 0 15.846789 16.041827v71.042374zM774.984951 112.978341H238.92465a48.759351 48.759351 0 0 0-50.856003 45.492474v819.986001A48.759351 48.759351 0 0 0 238.92465 1023.94929h661.030517A48.759351 48.759351 0 0 0 950.81117 978.456816V286.51287zM442.39742 892.299043a17.9922 17.9922 0 0 1-18.235997 17.748404H358.872652a17.9922 17.9922 0 0 1-18.235997-17.748404v-101.126893a17.9922 17.9922 0 0 1 18.235997-17.748404H424.210182a17.9922 17.9922 0 0 1 18.235997 17.748404V892.299043z m178.069149 3.120599a15.066639 15.066639 0 0 1-15.212918 14.627805H533.723684a15.115399 15.115399 0 0 1-15.212918-14.627805V423.770443a15.115399 15.115399 0 0 1 15.212918-14.627806h71.529967a15.115399 15.115399 0 0 1 15.212918 14.627806v471.600439z m177.92287 3.900748a11.068373 11.068373 0 0 1-11.21465 10.727057h-79.380223a11.068373 11.068373 0 0 1-11.214651-10.922095v-251.451971a11.068373 11.068373 0 0 1 11.214651-10.922095h79.331463a11.117132 11.117132 0 0 1 11.214651 10.922095v251.647009z" fill="#ffffff" p-id="3203"></path></svg></svg>
                                        <span style="font-size: 14px; letter-spacing: 2px; text-indent: 4px">辅导员信息管理</span>
                                    </button>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="javascript:;">
                                    <button id="searchButton" type="button" class="btn btn-primary btn-lg">
                                        <svg t="1607545578118" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3206" width="24" height="24">
                                            <path d="M743.416465 440.95258a25.884142 25.884142 0 0 0-25.542814 26.168584c0 32.653841-6.143928 65.819677-18.88689 95.685994l-0.625771 1.251541a241.092298 241.092298 0 0 1-54.15759 80.383063 245.586468 245.586468 0 0 1-81.236386 54.214478 240.637193 240.637193 0 0 1-95.856659 19.171332 25.770366 25.770366 0 0 0 0 51.483844c38.968434 0 78.505751-7.67991 115.653762-22.925955a295.477442 295.477442 0 0 0 97.961523-65.762788 298.321853 298.321853 0 0 0 64.795689-96.539318l0.62577-1.763535a295.989436 295.989436 0 0 0 23.153508-115.198656 25.770366 25.770366 0 0 0-25.884142-26.168584M349.63617 188.823966c-18.545561 7.338581-35.89647 16.440697-51.085626 27.078795-16.497585 11.263869-31.686741 22.812178-45.055475 36.806682a326.822854 326.822854 0 0 0-37.148011 45.112362v0.625771a246.212239 246.212239 0 0 0-27.363236 51.199403c-6.08704 12.458521 0 27.363236 13.368733 33.165835a25.144596 25.144596 0 0 0 33.165835-13.710062c6.371481-14.904715 13.994503-29.240548 23.437949-42.666169a263.392483 263.392483 0 0 1 30.4352-37.148011c11.54831-10.922539 24.34816-21.901967 37.773782-31.004083 13.311845-8.874563 27.932119-16.497585 41.926622-22.584625a25.827254 25.827254 0 0 0 13.425621-34.132936c-4.892387-12.742962-20.081543-18.886891-32.881394-12.742962m387.067484 548.174938l-0.568882 0.568882a395.942047 395.942047 0 0 1-122.992343 81.919044 382.459538 382.459538 0 0 1-146.032074 29.297436 380.752891 380.752891 0 0 1-146.088962-29.297436 395.430053 395.430053 0 0 1-123.845667-82.487926 381.776879 381.776879 0 0 1-82.487926-124.300772l-1.194653-2.104864a371.935216 371.935216 0 0 1-27.989007-143.472104c0-52.337167 10.353657-101.659258 29.18366-146.145851a380.866668 380.866668 0 0 1 82.487926-124.243884 381.549326 381.549326 0 0 1 269.934629-111.500921 388.546578 388.546578 0 0 1 146.032074 28.671665c46.307015 19.171332 88.290525 47.786109 123.561225 82.829256l2.161753 2.161753a375.291622 375.291622 0 0 1 80.610615 122.082131l0.9671 1.820423c17.919791 44.486592 27.647677 93.239801 27.647677 144.325428 0 51.199403-9.727887 100.578382-28.614777 145.633856a372.845428 372.845428 0 0 1-82.772368 124.243884m274.485687 213.78595l-185.28495-185.171173a478.145533 478.145533 0 0 0 72.703152-120.034155c22.527737-54.783361 35.2707-115.71065 35.2707-178.458362a466.483447 466.483447 0 0 0-34.360488-176.581051l-0.910212-2.844412a467.96254 467.96254 0 0 0-99.213065-148.819597l-2.104864-2.446193A480.022844 480.022844 0 0 0 646.023823 34.656876h-0.568882A459.144866 459.144866 0 0 0 467.110355 0.011947 465.231906 465.231906 0 0 0 35.613167 288.37836 460.055077 460.055077 0 0 0 0.001138 467.064275c0 62.121942 12.458521 121.570137 34.360488 176.069057l1.251541 2.446194c23.437949 56.888225 58.424207 108.65651 101.033488 151.664008h0.568882a471.774052 471.774052 0 0 0 151.550232 101.716147 468.872752 468.872752 0 0 0 357.258054 0 487.361425 487.361425 0 0 0 119.294608-72.475599l185.341838 185.171173c16.725138 16.497585 43.803933 16.497585 60.529072-0.341329 17.066468-16.725138 17.066468-43.803933 0-60.58596" fill="#ffffff" p-id="3207"></path></svg><span style="font-size: 14px; letter-spacing: 2px; text-indent: 4px">  查 询</span>
                                    </button>
                                </a>
                            </li>
                        </ul>
                        <ul class="navbar-nav">
                            <li class="nav-item">
                                <asp:Button ID="Button1" class="btn btn-primary btn-lg" runat="server" Text="注销" OnClick="Button1_Click" />
                            </li>
                        </ul>
                    </div>
                </div>

            </nav>
            <div class="container">
                <div class="row">
                    <div class="col">
                        <hr />
                    </div>
                </div>
                <!-- row -->
                <div class="row tm-content-row" id="chartCount">
                    <div class="col-12 tm-block-col">
                        <div class="tm-bg-primary-dark tm-block tm-block-taller tm-block-scroll">
                            <h2 class="tm-block-title" style="font-size: 20px; padding: 10px; background-color: #000; background: linear-gradient(45deg, #151515, #0000); margin-top: -40px;">学生信息</h2>
                            <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="false" OnSelectedIndexChanged="GridView1_SelectedIndexChanged" OnRowCommand="GridView1_RowCommand">
                                <Columns>
                                    <asp:BoundField DataField="_Name"
                                        ReadOnly="true"
                                        HeaderText="姓名" />
                                    <asp:BoundField DataField="ID_Student"
                                        ReadOnly="true"
                                        HeaderText="身份证信息" />
                                    <asp:BoundField DataField="_Sex"
                                        ConvertEmptyStringToNull="true"
                                        HeaderText="性别" />
                                    <asp:BoundField DataField="_Age"
                                        ConvertEmptyStringToNull="true"
                                        HeaderText="年龄" />
                                    <asp:BoundField DataField="_Nation"
                                        ConvertEmptyStringToNull="true"
                                        HeaderText="民族" />
                                    <asp:BoundField DataField="_Address"
                                        ConvertEmptyStringToNull="true"
                                        HeaderText="地址" />
                                    <asp:BoundField DataField="_PhoneNum"
                                        ConvertEmptyStringToNull="true"
                                        HeaderText="手机号" />
                                    <asp:BoundField DataField="_Email"
                                        ConvertEmptyStringToNull="true"
                                        HeaderText="邮箱" />
                                    <asp:BoundField DataField="_SH"
                                        ConvertEmptyStringToNull="true"
                                        HeaderText="是否已经审核" />
                                    <asp:ButtonField HeaderText="审核" ButtonType="Button" Text="审核" CommandName="SH" />
                                    <asp:BoundField DataField="_Comment"
                                        ConvertEmptyStringToNull="true"
                                        HeaderText="反馈信息" />
                                </Columns>
                            </asp:GridView>
                        </div>
                    </div>
                </div>
            </div>
            <footer class="tm-footer row tm-mt-small">
                <div class="col-12 font-weight-light">
                    <%--<p class="text-center text-white mb-0 px-4 small">Copyright &copy; 2019.Company name All rights reserved.<a href="javascript:;">兴义师范学院缴费系统;</a></p>--%>
                </div>
            </footer>
        </div>
        <div class="" id="searchView1">
            <svg id="close1" t="1607649420226" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3172" width="30" height="30">
                <path d="M921.856 987.008c-13.80352 0-26.7264-5.31456-36.37248-14.96064L512 598.56384l-373.48352 373.48352c-9.64608 9.64608-22.56384 14.96064-36.37248 14.96064s-26.7264-5.31456-36.37248-14.96064l-2.048-2.048c-9.64608-9.64608-14.95552-22.55872-14.95552-36.37248 0-13.80352 5.31456-26.7264 14.95552-36.37248L437.21216 523.776 63.72352 150.29248c-9.88672-9.6256-15.33952-22.54848-15.33952-36.37248s5.4528-26.74688 15.36512-36.39296l2.02752-2.02752c9.64608-9.64608 22.55872-14.95552 36.37248-14.95552 13.80864 0 26.7264 5.31456 36.37248 14.96064L512 449.49504l373.48352-373.99552c9.64608-9.64608 22.56384-14.96064 36.37248-14.96064s26.7264 5.31456 36.37248 14.96064l2.048 2.04288c20.05504 20.05504 20.05504 52.68992 0 72.74496L586.78784 523.776l373.48352 373.48352c9.64608 9.64608 14.96064 22.56384 14.96064 36.37248s-5.31456 26.7264-14.96064 36.37248l-2.048 2.048c-9.64096 9.64096-22.56384 14.95552-36.36736 14.95552z" p-id="3173" fill="#ffffff"></path><path d="M921.856 63.104c12.544 0 25.088 4.736 34.56 14.208l2.048 2.048c18.944 18.944 18.944 50.176 0 69.12l-375.296 375.296 375.296 375.296c18.944 18.944 18.944 50.176 0 69.12l-2.048 2.048c-9.472 9.472-22.016 14.208-34.56 14.208s-25.088-4.736-34.56-14.208L512 594.944l-375.296 375.296c-9.472 9.472-22.016 14.208-34.56 14.208s-25.088-4.736-34.56-14.208l-2.048-2.048c-18.944-18.944-18.944-50.176 0-69.12l375.296-375.296L65.536 148.48c-19.456-18.944-19.456-50.176 0-69.12l2.048-2.048c9.472-9.472 22.016-14.208 34.56-14.208s25.088 4.736 34.56 14.208L512 453.12l375.296-375.808c9.472-9.472 22.016-14.208 34.56-14.208m0-5.12c-14.4896 0-28.05248 5.5808-38.17984 15.70816L512 445.8752 140.32896 73.69216c-10.13248-10.13248-23.69024-15.70816-38.18496-15.70816s-28.05248 5.5808-38.17984 15.70816l-2.048 2.048A52.91008 52.91008 0 0 0 45.824 113.92c0 14.5152 5.7344 28.09344 16.13824 38.22592L433.59232 523.776l-371.67616 371.67616c-10.12736 10.12736-15.70816 23.69024-15.70816 38.17984s5.5808 28.05248 15.70816 38.17984l2.048 2.048c10.12736 10.12736 23.69024 15.70816 38.17984 15.70816s28.05248-5.5808 38.17984-15.70816L512 602.18368l371.67616 371.67616c10.12736 10.12736 23.69024 15.70816 38.17984 15.70816s28.05248-5.5808 38.17984-15.70816l2.048-2.048c10.12736-10.12736 15.70816-23.69024 15.70816-38.17984s-5.5808-28.05248-15.70816-38.17984L590.40768 523.776l371.67616-371.67616c21.05344-21.05344 21.05344-55.30624 0-76.35968l-2.048-2.048c-10.12736-10.12736-23.69024-15.70816-38.17984-15.70816z" p-id="3174" fill="#ffffff"></path></svg>
            <div class="container">
                <iframe id="inlineFrameExample"
                    title="Inline Frame Example"
                    style="margin: 0px auto; width: 100%; height: 100vh; background-color: rgba(255, 255, 255,0.1)"
                    name="Iframe"
                    frameborder="no"
                    src="./Search.aspx"></iframe>
            </div>
        </div>

        <div class="" id="searchView2">
            <svg id="close2" t="1607649420226" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3172" width="30" height="30">
                <path d="M921.856 987.008c-13.80352 0-26.7264-5.31456-36.37248-14.96064L512 598.56384l-373.48352 373.48352c-9.64608 9.64608-22.56384 14.96064-36.37248 14.96064s-26.7264-5.31456-36.37248-14.96064l-2.048-2.048c-9.64608-9.64608-14.95552-22.55872-14.95552-36.37248 0-13.80352 5.31456-26.7264 14.95552-36.37248L437.21216 523.776 63.72352 150.29248c-9.88672-9.6256-15.33952-22.54848-15.33952-36.37248s5.4528-26.74688 15.36512-36.39296l2.02752-2.02752c9.64608-9.64608 22.55872-14.95552 36.37248-14.95552 13.80864 0 26.7264 5.31456 36.37248 14.96064L512 449.49504l373.48352-373.99552c9.64608-9.64608 22.56384-14.96064 36.37248-14.96064s26.7264 5.31456 36.37248 14.96064l2.048 2.04288c20.05504 20.05504 20.05504 52.68992 0 72.74496L586.78784 523.776l373.48352 373.48352c9.64608 9.64608 14.96064 22.56384 14.96064 36.37248s-5.31456 26.7264-14.96064 36.37248l-2.048 2.048c-9.64096 9.64096-22.56384 14.95552-36.36736 14.95552z" p-id="3173" fill="#ffffff"></path><path d="M921.856 63.104c12.544 0 25.088 4.736 34.56 14.208l2.048 2.048c18.944 18.944 18.944 50.176 0 69.12l-375.296 375.296 375.296 375.296c18.944 18.944 18.944 50.176 0 69.12l-2.048 2.048c-9.472 9.472-22.016 14.208-34.56 14.208s-25.088-4.736-34.56-14.208L512 594.944l-375.296 375.296c-9.472 9.472-22.016 14.208-34.56 14.208s-25.088-4.736-34.56-14.208l-2.048-2.048c-18.944-18.944-18.944-50.176 0-69.12l375.296-375.296L65.536 148.48c-19.456-18.944-19.456-50.176 0-69.12l2.048-2.048c9.472-9.472 22.016-14.208 34.56-14.208s25.088 4.736 34.56 14.208L512 453.12l375.296-375.808c9.472-9.472 22.016-14.208 34.56-14.208m0-5.12c-14.4896 0-28.05248 5.5808-38.17984 15.70816L512 445.8752 140.32896 73.69216c-10.13248-10.13248-23.69024-15.70816-38.18496-15.70816s-28.05248 5.5808-38.17984 15.70816l-2.048 2.048A52.91008 52.91008 0 0 0 45.824 113.92c0 14.5152 5.7344 28.09344 16.13824 38.22592L433.59232 523.776l-371.67616 371.67616c-10.12736 10.12736-15.70816 23.69024-15.70816 38.17984s5.5808 28.05248 15.70816 38.17984l2.048 2.048c10.12736 10.12736 23.69024 15.70816 38.17984 15.70816s28.05248-5.5808 38.17984-15.70816L512 602.18368l371.67616 371.67616c10.12736 10.12736 23.69024 15.70816 38.17984 15.70816s28.05248-5.5808 38.17984-15.70816l2.048-2.048c10.12736-10.12736 15.70816-23.69024 15.70816-38.17984s-5.5808-28.05248-15.70816-38.17984L590.40768 523.776l371.67616-371.67616c21.05344-21.05344 21.05344-55.30624 0-76.35968l-2.048-2.048c-10.12736-10.12736-23.69024-15.70816-38.17984-15.70816z" p-id="3174" fill="#ffffff"></path></svg>
            <div class="container">
                <iframe id="inlineFrameExample"
                    title="Inline Frame Example"
                    style="margin: 0px auto; width: 100%; height: 100vh; background-color: rgba(255, 255, 255,0.1)"
                    name="Iframe"
                    frameborder="no"
                    src="./yourselfInfo.aspx"></iframe>
            </div>
        </div>
        <%--数据统计--%>
        <div class="" id="searchView3">
            <svg id="close3" t="1607649420226" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3172" width="30" height="30">
                <path d="M921.856 987.008c-13.80352 0-26.7264-5.31456-36.37248-14.96064L512 598.56384l-373.48352 373.48352c-9.64608 9.64608-22.56384 14.96064-36.37248 14.96064s-26.7264-5.31456-36.37248-14.96064l-2.048-2.048c-9.64608-9.64608-14.95552-22.55872-14.95552-36.37248 0-13.80352 5.31456-26.7264 14.95552-36.37248L437.21216 523.776 63.72352 150.29248c-9.88672-9.6256-15.33952-22.54848-15.33952-36.37248s5.4528-26.74688 15.36512-36.39296l2.02752-2.02752c9.64608-9.64608 22.55872-14.95552 36.37248-14.95552 13.80864 0 26.7264 5.31456 36.37248 14.96064L512 449.49504l373.48352-373.99552c9.64608-9.64608 22.56384-14.96064 36.37248-14.96064s26.7264 5.31456 36.37248 14.96064l2.048 2.04288c20.05504 20.05504 20.05504 52.68992 0 72.74496L586.78784 523.776l373.48352 373.48352c9.64608 9.64608 14.96064 22.56384 14.96064 36.37248s-5.31456 26.7264-14.96064 36.37248l-2.048 2.048c-9.64096 9.64096-22.56384 14.95552-36.36736 14.95552z" p-id="3173" fill="#ffffff"></path><path d="M921.856 63.104c12.544 0 25.088 4.736 34.56 14.208l2.048 2.048c18.944 18.944 18.944 50.176 0 69.12l-375.296 375.296 375.296 375.296c18.944 18.944 18.944 50.176 0 69.12l-2.048 2.048c-9.472 9.472-22.016 14.208-34.56 14.208s-25.088-4.736-34.56-14.208L512 594.944l-375.296 375.296c-9.472 9.472-22.016 14.208-34.56 14.208s-25.088-4.736-34.56-14.208l-2.048-2.048c-18.944-18.944-18.944-50.176 0-69.12l375.296-375.296L65.536 148.48c-19.456-18.944-19.456-50.176 0-69.12l2.048-2.048c9.472-9.472 22.016-14.208 34.56-14.208s25.088 4.736 34.56 14.208L512 453.12l375.296-375.808c9.472-9.472 22.016-14.208 34.56-14.208m0-5.12c-14.4896 0-28.05248 5.5808-38.17984 15.70816L512 445.8752 140.32896 73.69216c-10.13248-10.13248-23.69024-15.70816-38.18496-15.70816s-28.05248 5.5808-38.17984 15.70816l-2.048 2.048A52.91008 52.91008 0 0 0 45.824 113.92c0 14.5152 5.7344 28.09344 16.13824 38.22592L433.59232 523.776l-371.67616 371.67616c-10.12736 10.12736-15.70816 23.69024-15.70816 38.17984s5.5808 28.05248 15.70816 38.17984l2.048 2.048c10.12736 10.12736 23.69024 15.70816 38.17984 15.70816s28.05248-5.5808 38.17984-15.70816L512 602.18368l371.67616 371.67616c10.12736 10.12736 23.69024 15.70816 38.17984 15.70816s28.05248-5.5808 38.17984-15.70816l2.048-2.048c10.12736-10.12736 15.70816-23.69024 15.70816-38.17984s-5.5808-28.05248-15.70816-38.17984L590.40768 523.776l371.67616-371.67616c21.05344-21.05344 21.05344-55.30624 0-76.35968l-2.048-2.048c-10.12736-10.12736-23.69024-15.70816-38.17984-15.70816z" p-id="3174" fill="#ffffff"></path></svg>
            <div class="container">
                <iframe id="inlineFrameExample"
                    title="Inline Frame Example"
                    style="margin: 0px auto; width: 100%; height: 100vh; background-color: rgba(255, 255, 255,0.1)"
                    name="Iframe"
                    frameborder="no"
                    src="./counselor.aspx"></iframe>
            </div>
        </div>
        <div class="" id="searchView4">
            <svg id="close4" t="1607649420226" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3172" width="30" height="30">
                <path d="M921.856 987.008c-13.80352 0-26.7264-5.31456-36.37248-14.96064L512 598.56384l-373.48352 373.48352c-9.64608 9.64608-22.56384 14.96064-36.37248 14.96064s-26.7264-5.31456-36.37248-14.96064l-2.048-2.048c-9.64608-9.64608-14.95552-22.55872-14.95552-36.37248 0-13.80352 5.31456-26.7264 14.95552-36.37248L437.21216 523.776 63.72352 150.29248c-9.88672-9.6256-15.33952-22.54848-15.33952-36.37248s5.4528-26.74688 15.36512-36.39296l2.02752-2.02752c9.64608-9.64608 22.55872-14.95552 36.37248-14.95552 13.80864 0 26.7264 5.31456 36.37248 14.96064L512 449.49504l373.48352-373.99552c9.64608-9.64608 22.56384-14.96064 36.37248-14.96064s26.7264 5.31456 36.37248 14.96064l2.048 2.04288c20.05504 20.05504 20.05504 52.68992 0 72.74496L586.78784 523.776l373.48352 373.48352c9.64608 9.64608 14.96064 22.56384 14.96064 36.37248s-5.31456 26.7264-14.96064 36.37248l-2.048 2.048c-9.64096 9.64096-22.56384 14.95552-36.36736 14.95552z" p-id="3173" fill="#ffffff"></path><path d="M921.856 63.104c12.544 0 25.088 4.736 34.56 14.208l2.048 2.048c18.944 18.944 18.944 50.176 0 69.12l-375.296 375.296 375.296 375.296c18.944 18.944 18.944 50.176 0 69.12l-2.048 2.048c-9.472 9.472-22.016 14.208-34.56 14.208s-25.088-4.736-34.56-14.208L512 594.944l-375.296 375.296c-9.472 9.472-22.016 14.208-34.56 14.208s-25.088-4.736-34.56-14.208l-2.048-2.048c-18.944-18.944-18.944-50.176 0-69.12l375.296-375.296L65.536 148.48c-19.456-18.944-19.456-50.176 0-69.12l2.048-2.048c9.472-9.472 22.016-14.208 34.56-14.208s25.088 4.736 34.56 14.208L512 453.12l375.296-375.808c9.472-9.472 22.016-14.208 34.56-14.208m0-5.12c-14.4896 0-28.05248 5.5808-38.17984 15.70816L512 445.8752 140.32896 73.69216c-10.13248-10.13248-23.69024-15.70816-38.18496-15.70816s-28.05248 5.5808-38.17984 15.70816l-2.048 2.048A52.91008 52.91008 0 0 0 45.824 113.92c0 14.5152 5.7344 28.09344 16.13824 38.22592L433.59232 523.776l-371.67616 371.67616c-10.12736 10.12736-15.70816 23.69024-15.70816 38.17984s5.5808 28.05248 15.70816 38.17984l2.048 2.048c10.12736 10.12736 23.69024 15.70816 38.17984 15.70816s28.05248-5.5808 38.17984-15.70816L512 602.18368l371.67616 371.67616c10.12736 10.12736 23.69024 15.70816 38.17984 15.70816s28.05248-5.5808 38.17984-15.70816l2.048-2.048c10.12736-10.12736 15.70816-23.69024 15.70816-38.17984s-5.5808-28.05248-15.70816-38.17984L590.40768 523.776l371.67616-371.67616c21.05344-21.05344 21.05344-55.30624 0-76.35968l-2.048-2.048c-10.12736-10.12736-23.69024-15.70816-38.17984-15.70816z" p-id="3174" fill="#ffffff"></path></svg>
            <div class="container">
                <iframe id="inlineFrameExample"
                    title="Inline Frame Example"
                    style="margin: 0px auto; width: 100%; height: 100vh; background-color: rgba(255, 255, 255,0.1)"
                    name="Iframe"
                    frameborder="no"
                    src="./counselorDataStatistics.aspx"></iframe>
            </div>
        </div>
    </form>
    <script src="../js/jquery-3.3.1.min.js"></script>
    <!-- https://jquery.com/download/ -->
    <script src="../js/moment.min.js"></script>
    <!-- https://momentjs.com/ -->
    <script src="../js/search.js"></script>
    <script src="../js/search1.js"></script>
</body>

</html>
